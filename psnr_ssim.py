...
@torch.no_grad()
def calculate_psnr_torch(img1, img2, crop_border, test_y_channel=False):
    """Calculate PSNR (Peak Signal-to-Noise Ratio).

    Args:
        img1: (torch.Tensor) with shape (N, C, H, W) on CUDA.
        img2: (torch.Tensor) with shape (N, C, H, W) on CUDA.
        crop_border: (int) Crop border.
    """
    assert test_y_channel is False, 'test_y_channel is not supported in calculate_psnr_torch.'
    assert img1.shape == img2.shape, f'Image shapes are different: {img1.shape}, {img2.shape}.'

    # check if there are nan values in the images
    if torch.isnan(img1).any() or torch.isnan(img2).any():
        raise ValueError('There are nan values in the images.')
    ...
    def _psnr(img1, img2):
        mse = torch.mean(torch.pow(img1 - img2, 2), dim=(1, 2, 3))
        if torch.any(mse == 0):
            raise ValueError("One or more images in the batch are identical, division by 0.")
        max_value = torch.where(img1.amax() <= 1, torch.tensor(1.0, device=img1.device), torch.tensor(255.0, device=img1.device))
